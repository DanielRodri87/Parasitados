...
class Questions {
	...
	static Future<Questions> fromCsvFile() async {
		...
		final rows = const CsvToListConverter(eol: '\n', fieldDelimiter: ',').convert(csvString, shouldParseNumbers: false);
		...
		for (int i = 1; i < rows.length; i++) {
			...
			if (row[7] != null && row[7].toString().trim().isNotEmpty) opcoes.add(row[7]);
			final respostaLetra = row[8]?.toString().toUpperCase() ?? '';
			final respostaCorreta = respostaLetra.isNotEmpty ? respostaLetra.codeUnitAt(0) - 'A'.codeUnitAt(0) + 1 : 0;
