  ...
  void initState() {
	...
	Future<void> _loadPlayers() async {
		...
		if (players.isNotEmpty) {
			setState(() {
				player1Name = ((players[0]['nome1'] as String).isEmpty) ? userSharedPref!['nome'] : players[0]['nome1'];
  	...
  	Future<void> _checkForLooser() async {
		if (player1Animals.isEmpty) {
			...
			if(widget.modeGame == TypeModeGame.umJogador){
				...
				if(
					await RankingDatabase().inserirRanking(
    ...
    return Scaffold(
      body: Stack(
        children: [
		  ...
		  if (gameEnded) ...[
			if (widget.modeGame == TypeModeGame.doisJogador) _buildVictoryScreen(),
		  ],
		],
	  ),
	);
