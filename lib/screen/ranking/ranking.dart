...
class RankingScreen extends StatelessWidget {
  ...
  Future<void> _loadRanking(BuildContext context) async {
    await Provider.of<RankingProvider>(
      context,
      listen: false,
    ).getRankingPorId();
  }
      ...
      builder: (context, asyncSnapshot) {
        return Consumer<RankingProvider>(
          builder: (context, value, child) {
            ...
            final tempoRealizado =
                tempoRealizadoRaw is int
                  ...
                  SafeArea(
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 16),
                        // Pulga
                        Image.asset('assets/images/pulgo.png', height: 200),
                        const SizedBox(height: 16),
                        // LadoPar e Texto
                        SizedBox(
                          height: 80,
                            ...
                            child: Wrap(
                              ...
                              children: [
                                ...
                                MetricCard(
                                  ...
                                  label:
                                      '${((value.ranking?['taxa_de_acerto'] ?? 0.0) is int ? (value.ranking?['taxa_de_acerto'] ?? 0.0).toDouble() : (value.ranking?['taxa_de_acerto'] ?? 0.0)).toStringAsFixed(0)}%',
                                  sublabel: 'Taxa de acertos',
                                ),
                                MetricCard(
                                  icon: 'assets/images/raiorank.png',
                                  label:
                                      '${value.ranking?['qtd_acertos']} Acertos',
                                  sublabel: 'Desempenho',
                                ),
                                MetricCard(
