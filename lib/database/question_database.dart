...
class QuestionDatabase {
  ...
  Future<int> addQuestion(
    ...
    if (pgDatabase!.usePostgresLocal) {
      ...
      final response =
          await pgDatabase!.supabase!.from('questions').upsert({
            'id': questionId,
            ...question.toJson(),
          }).select();
  ...
  Future<Map<int, dynamic>> getAllQuestions() async {
    ...
    if (pgDatabase!.usePostgresLocal) {
      ...
      final result = await pgDatabase!.pgHelper!.queryAll(
  ...
  Future<Map<String, dynamic>?> getQuestion(int id) async {
    if (pgDatabase!.usePostgresLocal) {
      ...
      final List data = await pgDatabase!.supabase!
          .from('questions')
          .select()
          .eq('id', id);
      if (data.isNotEmpty) {
  ...
  Future<Questions> databaseToLocal() async {
    ...
    questions.questoes = Map.fromEntries(
      questoes.entries.map(
        (entry) =>
            MapEntry(entry.key, Question.fromJson(entry.key, entry.value)),
      ),
    );
    return questions;
  }
  ...
  Future<void> importAllFromCsv() async {
    if (pgDatabase!.usePostgresLocal) {
      ...
      for (final question in questions.questoes.values) {
        await addQuestion(questions, question, allFunction: true);
      }
